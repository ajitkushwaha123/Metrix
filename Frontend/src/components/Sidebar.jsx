...
const Sidebar = () => {
  ...
  return (
    <div className="hover:w-[296px] w-[88px] pb-[30px] h-screen font-poppins group bg-[#fff]">
      ...
      <div className="flex flex-col justify-center items-center">
        {/* Navigation Links */}
        <ul className={`flex flex-col justify-center items-center font-poppins`}>
  {navLinks.map((nav, index) => (
    ...
    >
      {nav.icon}
      <span className="hidden group-hover:block ml-[10px]"><NavLink to={nav.link}>{nav.title}</NavLink></span>
    </li>
  ))}
</ul>

        <ul className="font-poppins flex justify-center items-center flex-col absolute bottom-0">
        <NavLink to='/support'><li className="flex items-center h-[53px] bg-[#eff0f0] rounded-lg group-hover:w-[233px] text-[#212227] text-[16px] px-[15px] group-hover:px-[30px]">
